...
plt.plot(x_data, initial_guess[5] * np.exp(-1 * ((x_data - initial_guess[0]) ** 2) / (2 * initial_guess[1] ** 2)),
         'g--', label='Gaussian')
plt.plot(x_data,
         initial_guess[4] * gauss_expo_convolution(x_data, initial_guess[0], initial_guess[1], initial_guess[2]), 'y--',
         label='Exponential 1')
plt.plot(x_data,
         initial_guess[5] * gauss_expo_convolution(x_data, initial_guess[0], initial_guess[1], initial_guess[3]), 'b--',
         label='Exponential 2')
plt.legend()
plt.xlabel('Channel')
plt.ylabel('Counts')
