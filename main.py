...
def E_out_fluctuated(theta, mc2, a):
    """
...
energy_calculated = [E_out_fluctuated(angle, mc2=0.510998, a=0) for angle in theta_list]
...
def neg_log_likelihood(params, energy_simulated, scale):
    mc2, a = params
    energy_calculated = E_out_fluctuated(theta_list, mc2, a)
...
result = minimize(neg_log_likelihood, [0.510998, 0], args=(energy_simulated, scale))
...
plt.plot(theta_list, E_out_fluctuated(theta_list, mc2=mc2, a=0), label='fit', color='b')
...
def simulate_likelihood(sigma):
    ...
    for i in range(n_iter):
        ...
        energy_simulated = np.add(energy_calculated, energy_fluctuation)
        # perform the maximum likelihood fit
        result = minimize(neg_log_likelihood, [0.5, 0], args=(energy_simulated, scale))
        # retrieve the value of mc2
        mc2 = result.x[0]
